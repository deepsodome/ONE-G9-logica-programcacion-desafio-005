...
let numeroSecreto = Math.floor(Math.random(numeroRangoIni)*numeroRangoFin)+1; // crea variable Y asigna el número a encontrar
...
while(numeroSecreto != numeroUsuario){  // repite hasta el maximo numero de intentos
   ...
   if (numeroUsuario < numeroRangoIni || numeroUsuario > numeroRangoFin){       //compara compara que el numero de usuario sea mayor a rango inicial y menor al rango final
        alert(`¡Número inválido! \nEl rango es entre ${numeroRangoIni} y ${numeroRangoFin} \nFIN DEL JUEGO \nDa clic en -Aceptar- y pulsa la tecla -F5- para intentarlo de nuevo.`);        //muestra error numero usuario es menor a rango inicial y mayor a rango final
        break;      //sale de loop
    }
